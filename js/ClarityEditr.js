...
Clarity.prototype.handle_draw = function () {
  if (this.isDrawing && !this.square_tool_active) {
    ...
    if (
      worldY > this.current_map.data.length - 1 ||
      worldX > this.current_map.data[0].length - 1
    ) {
      return; // Don't let the user place blocks outside the array
    }

    this.current_map.data[worldY][worldX] =
      this.current_map.keys[this.selectedTile]; // Selection
...
Clarity.prototype.update_player = function () {
  ...
  if (this.viewportControls.space) {
    ...
    this.player.loc.x =
      this.cameraToWorldSpace(this.mouse.x, this.camera.x) + 20;
    this.player.loc.y =
      this.cameraToWorldSpace(this.mouse.y, this.camera.y) + 20;
    this.player.vel.x = 0;
    this.player.vel.y = 0;
  }
...
Clarity.prototype.draw = function (context) {
  ...
  if (this.square_tool_active) {
    ...
    if (this.square_tool_positions.length >= 1) {
      ...
      if (
        worldY2 > this.current_map.data.length - 1 ||
        worldX2 > this.current_map.data[0].length - 1
      ) {
        return; // Don't let the user place blocks outside the array
      }
...
Clarity.prototype.screenToWorldSpace = function (coord, cameraAxis) {
  return (
    Math.floor(
      (coord - this.tile_size * 2 + Math.trunc(cameraAxis)) / this.tile_size
    ) + 2
  );
};
